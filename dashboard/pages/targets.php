...
<div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
				<div class="panel-body">
                    <h3 class="page-header">Past Targets</h3>
					<?php
					
						
						$target_records = mysql_query("SELECT * FROM target WHERE cid ='".$cid."' AND status <> '0'");
						
						if($record = mysql_fetch_assoc($target_records))
						{
						?>
	...
	while ($record = mysql_fetch_assoc($target_records)) {
	
		if($record['status'] == 1)
		{
			$comment = "Great,You achieved the target !";
			}
		else $comment ="You should spend less";	
    echo '<tr>';
   
        echo '<td>' . $record['target_name'] . '</td>';
		echo '<td>' . $record['target_money'] . '</td>';
		echo '<td>' . $record['curren_money'] . '</td>';
...
echo "</table>";
				...
				echo "<br/> <h2>No past targets record</h2><br/>";
